...
# Copy model files from models/ if not already there
...
if not arjuna_engine.exists():
    # Write it (content from previous script)
...
if "scores" in krishna_content:
    # Insert arjuna score in the scores dict
    scores_line = '"scores": {'
    if scores_line in krishna_content:
        # Insert after bhishma, hanuman, shakuni lines (we'll find a robust way)
        # Simpler: replace the whole scores dict building section
        # For now, we'll add a line after the existing scores dict (assuming it's already there)
        # Let's just add arjuna to the scores dict using string replacement
        # We'll find the line that closes the scores dict and add before it
        close_scores = '"scores": { ... }' - not easy. Instead, we'll do a targeted replacement.
        # We'll assume the scores dict is built like this:
        #   "scores": {
        #       "bhishma": round(bhishma_score, 3),
        #       "hanuman": round(hanuman_score, 3),
        #       "shakuni": round(shakuni_score, 3)
        #   },
        # We'll add "arjuna" line after shakuni.
        ...
        print("Warning: Could not find scores dict to add arjuna.")
